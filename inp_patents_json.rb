...
source_temp = File.read("test_2.txt")
source_temp_lines = File.readlines("test_2.txt")
...
def analyze(str)
 ...
 #end
 #puts str
 str.scan(/\#\d+[a-zA-Zа-яА-ЯҒғЎўҚқЪъЬьЁёЫыČŬËŽÈèĞğŜŝÛûÂâŠščŭëžьǎÈè\+\-\s\d,\.`]+/) do |info_to_put|
	...
	while(amount_of_files < seq_of_files.length)
		...
		#while(c < source_temp_lines.length)
			...
			if a.to_i == tag_num.to_i
				...
				#if str has 1 character exluding letters and numbers or empty then replace with a space
				str = str.sub(/[^a-zA-Zа-яА-ЯҒғЎўҚқЪъЬьЁёЫы\+\-\d]+/, " ")
		...
		res = con.exec_params("insert into inp_patents (inp_patent) values ($1)", [prep_temp])
